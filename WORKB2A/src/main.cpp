...
// --- NETWORK CREDENTIALS (REPLACE WITH YOURS) ---
...
// This function takes the data given from the UART communication from the Arduino and packages into a packet for wireless communication
...
void loop() {
  ...
  if(Serial2.available() >= sizeof(internal_payLoad)){
    ...
    if(Serial2.peek() == 0xAA){
      ...
      if(arduinoPayload_buffer.syncByte_2 == 0xBB){
        ...
        Serial.println("Valid packet received and read from buffer");
      }
      else{
  ...
  if (millis() - lastGroupDTick >= groupDInterval) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      // Map Group D requested parameters using your defined macros
      ThingSpeak.setField(ID_CHANNEL_RPM, rpm);
      ThingSpeak.setField(ID_CHANNEL_PITCH, pitchAngle);
      ThingSpeak.setField(ID_CHANNEL_POWER, power);
      ThingSpeak.setField(ID_CHANNEL_TEMPERATURE, temperature);

      // Write fields using the correct variable names defined at the top
      ThingSpeak.writeFields(ID_channel, writeAPIKey);
    }
  }
}